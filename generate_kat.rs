...
use quantum_crypto::symmetric::xchacha::{encrypt_data, generate_xchacha_nonce, XChaChaNonce};
...
fn main() {
    ...
    let enc_key = derive_subkey_hkdf(master_key, b"encryption", 32).unwrap();
    let mac_key = derive_subkey_hkdf(master_key, b"mac", 32).unwrap();
    ...
    let metadata_ciphertext =
        encrypt_data(enc_key.as_slice(), &metadata_nonce, metadata_plaintext, None).unwrap();
    ...
    println!("pub const SALT: &[u8] = &{:?};", derived.salt);
    println!("pub const PARAMS: &[u8] = &{:?};", derived.params);
    println!("pub const EXPECTED_MASTER_KEY: &[u8] = &{:?};", master_key);
    println!("pub const EXPECTED_ENC_KEY: &[u8] = &{:?};", enc_key.as_slice());
    println!("pub const EXPECTED_MAC_KEY: &[u8] = &{:?};", mac_key.as_slice());
    println!("pub const VAULT_PAYLOAD: &[u8] = &{:?};", vault_payload);
    println!("pub const HMAC_TAG: &[u8] = &{:?};", hmac_tag);
    println!("pub const METADATA_NONCE: &[u8] = &{:?};", metadata_nonce.as_bytes());
    println!("pub const METADATA_CIPHERTEXT: &[u8] = &{:?};", metadata_ciphertext);
    println!("pub const ITEM_NONCE: &[u8] = &{:?};", item_nonce.as_bytes());
    println!("pub const ITEM_CIPHERTEXT: &[u8] = &{:?};", item_ciphertext);
}
